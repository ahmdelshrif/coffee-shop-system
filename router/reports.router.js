...
const {addingExpenses_catchError}
      =require("../utils/valdetor/report.vildator")


router.route("/").get(protect,allowTO("manager",'admin'),Getreport)

router.route("/addingExpenses").post(protect,allowTO("manager"),addingExpenses_catchError,addingExpenses)
router.route("/get_Best_selling_products").get(
    protect,allowTO("manager"),get_Best_selling_products)
